"""
This module stands for "features automatically generated". It
"""
...
def get_daily_volume(df, days_for_average=14, ratio=1 / 50):
    """
    ...
    # Drop last entry because maybe it's an incomplete day. Documentation and google is unclear.
    df = df[:-1]
...
def create_volume_bars(df, volume_for_new_candle=None):
    """
...
def reach_bar(df, atr_mult, nr_of_bars_for_vertical, which='top'):
    """
...
def will_reach_top_bar(df, atr_mult, nr_of_bars_for_vertical):
    """
    :param df: volume_bars with index (as date_time), open, close, high, low, atr and volume
    :param atr_mult: multiplier for the atr
    :param nr_of_bars_for_vertical: after how many bars the vertical bar will be produced.
...
def will_reach_bot_bar(df, atr_mult, nr_of_bars_for_vertical):
    """
    :param df: volume_bars with index (as date_time), open, close, high, low, atr and volume
    :param atr_mult: multiplier for the atr
    :param nr_of_bars_for_vertical: after how many bars the vertical bar will be produced.
...
def apply_triple_bar_method(df, atr_mult=2.8, nr_of_bars_for_vertical=13,
                            period_atr=7):
    """
    ...
    for index, row in df.iterrows():
        if not first:
            ...
            if row['top_bar'] == 1:
                first = 'top'
        if row['bot_bar'] + row['bot_bar'] == 0:
            first = None
        if row['bot_bar'] + row['bot_bar'] > 1:
            if first == 'top':
